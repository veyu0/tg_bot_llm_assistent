...
class CaseParser:
    ...
    def parse_page(self, url: str) -> dict:
        ...
        try:
            ...
            problem_section = soup.find('h3', string=lambda t: t and 'Задача' in t)
            problem_solved = 'Unknown'
            if problem_section:
                target_div = client_section.find_next('div')
                if target_div:
                    problem_solved = target_div.text.strip()
            else:
                self.logger.warning(f"No problem section found on page: {url}")

            self.logger.info(f"Parsed {url}: Name='{project_name}', For='{for_whom}', Problem='{problem_solved[:50]}...'")
    ...
    def run(self, source_file: str = 'parsing_source.txt'):
        ...
        for url in urls:
            ...
            if data:
                self.db.add_project(data['url'], data['name'], data['for_whom'], data['problem_solved'])